...
class BuildEngineImpl : public BuildDBDelegate {
  ...
  struct TaskInfo {
    ...
    std::unordered_set<std::shared_ptr<RuleScanRequest>> deferredScanRequests;
    /// Requested dependencies sorted by batch sequence
    std::map<unsigned, DependencyKeyIDs> sequencedDependencyBatches;
  ...
  bool executeTasks(const KeyType& buildKey) {
    ...
    while (true) {
      ...
      while (true) {
        ...
        //
        request.taskInfo->sequencedDependencyBatches
            [request.taskInfo->dependencyBatchSequence.load()].push_back(
      ...
      while (true) {
        ...
        // Add all dependencies with barriers between the batches
        for (auto &dependencies : taskInfo->sequencedDependencyBatches) {
          ruleInfo->result.dependencies.addBarrier();
          ruleInfo->result.dependencies.append(dependencies.second);
        }
        taskInfo->sequencedDependencyBatches.clear();
